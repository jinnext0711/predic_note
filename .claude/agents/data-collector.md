...
## 出馬表確定チェック
5. 各レースの出馬表データは `entry_validator.validate_entries()` で自動検証される（fetcher内蔵）
6. `entry_status` が `unconfirmed` / `not_available` のレースはチームリーダーに報告し、予想対象から除外するよう伝達
7. 報告にはレースごとの確定状態（確定/一部未確定/未確定）を含める

## 進捗トラッキング（GitHub Issue）
8. データ収集の開始時に `issue_manager.get_or_create_issue()` で予想 Issue を作成/取得
9. 各レースのデータ収集完了後、`issue_manager.update_race_status()` でステータスを `data_collected` に更新
10. 出馬表未確定のレースは `skipped_unconfirmed` ステータスで更新
11. エラー発生時は `error` ステータスとエラーメッセージで更新
...
- `src/data/entry_validator.py` - 出馬表確定チェック
- `src/tracking/issue_manager.py` - GitHub Issue 進捗管理
